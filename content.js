...
const addStickyNote = function (noteData = {}) {
  ...
  container.innerHTML = `
    ...
    <textarea name="note" placeholder="Write your notes here!">${
      noteData.description || ""
    }</textarea>`;

  container.style.setProperty(
    "--color-primary",
    noteData.colorPrimary || "#FFD700"
  );
  container.style.setProperty(
    "--color-secondary",
    noteData.colorSecondary || "#FFFAE3"
  );
...
const saveNotesToStorage = function () {
  ...
  document.querySelectorAll(".note-container").forEach((note) => {
    ...
    const colorPrimary = getComputedStyle(note)
      .getPropertyValue("--color-primary")
      .trim();
    const colorSecondary = getComputedStyle(note)
